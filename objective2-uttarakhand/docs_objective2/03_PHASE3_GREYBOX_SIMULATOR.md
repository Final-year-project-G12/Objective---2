...
**Symptom**: Gate 1's very first run showed an above-pass residual —
above the 0.1% threshold (though below 0.5% warn in some cases).
...
(mimicking a real differential thermostat) before logging anything.

**Result**: energy residuals for the majority of Gate 1 cases are in the
range 0.00022–0.00277% — well inside the 0.1% pass threshold.
...
`T_pcm` (~10^300) without the fix.

**Root cause**: the PCM temperature is updated one sub-step behind the
water temperature (a semi-implicit/IMEX coupling). This is stable as long
as each sub-step is short relative to the PCM's own thermal time constant
`τ = m_pcm·cp/UA_eff`. The original sub-stepping rule only added extra
sub-steps when `T_pcm` was near the melting band — it never checked `τ`
itself, so an artificially large `UA` (from the 200× conductivity multiplier)
made `τ` shorter than one 5-minute sub-step, causing divergence.
...
property combination can never silently propagate `NaN`/`inf`.

**Result** (Gate 2, Uttarakhand run): the high-conductivity case gives a
*smaller* mean |T_w−T_pcm| gap than the nominal-conductivity case
(0.062 °C vs 0.245 °C) — the physically-correct direction — instead of
diverging.
...
to the 50 °C delivery target (matched to Objective 1's `T_DELIVERY_C`),
and `E_unmet` is the accumulated shortfall whenever delivered water falls
below 50 °C. This is the standard "fraction of ideal demand actually met at
target temperature" definition; there is **no auxiliary/backup heater
modeled**, so this project's solar fraction is stricter than a real
installed system's.
