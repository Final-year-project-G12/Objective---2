...
Ported from objective2-tamilnadu's src/design/retarget_tm.py (2026-09-14),
which found the same mismatch there: the real shortlisted PCMs (Tm in the
56.5-64C range, climate/delivery-anchored) did not beat plain tank, while
a synthetic PCM matched to the tank's actual operating range did. See
docs_objective2/12_TM_TARGET_RETARGETING.md for the Uttarakhand result.
...
def select_shortlist_for_target(feasibility_df: pd.DataFrame, pcm_db_df: pd.DataFrame,
    ...
    # Uttarakhand's feasibility_survivors_by_cluster.csv (Objective 1 schema)
    # has pass_latent_heat/pass_cycling/pass_supercooling/pass_corrosion but
    # no separate pass_safety column (that flag only exists in Tamil Nadu's
    # later Objective 1 pipeline revision) -- include it only if present, so
    # this script stays correct for either schema.
    non_tm_flags = [c for c in ("pass_latent_heat", "pass_cycling", "pass_supercooling",
                                 "pass_corrosion", "pass_safety") if c in df.columns]
    all_flags_pass = df[non_tm_flags].all(axis=1)
    df["new_passes_all"] = (
        df["new_pass_melting_window"] & all_flags_pass & df["name"].isin(complete)
    )
...
def _rewrite_yaml_preserving_header(raw_text: str, cfg: dict) -> str:
    ...
    header += (
        ...
        "# results/uttarakhand/tm_retargeting_report.csv for the full before/after\n"
