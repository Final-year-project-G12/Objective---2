...
def search_regime_pcm(state: str, regime_id: int, pcm_id, models: dict, feature_cols: list,
                       ...
                       feasibility_feature_cols: list = None):
    ...
    for target, model in models.items():
        if target == "feasibility":
            # Every row here already passed check_design(valid=True) above
            # (line "if not geom['valid']: continue"), so the classifier's
            # "geometry_was_computed" training feature (added in train.py to
            # handle NaN geometry on bounds_violation-rejected DOE rows) is
            # always 1 here -- these candidates were never rejected.
            X_feas = X.copy()
            if feasibility_feature_cols is not None and "geometry_was_computed" in feasibility_feature_cols:
                X_feas["geometry_was_computed"] = 1
                X_feas = X_feas[feasibility_feature_cols]
            proba = model.predict_proba(X_feas)
...
def search_all_pairs(state: str, top_n: int = 5):
    ...
    feasibility_feature_cols = saved.get("feasibility_feature_cols")  # None for older pickles saved before this fix
    ...
    for regime in cfg["regimes"]:
        ...
        for pcm_id in regime["pcm_shortlist"]:
            top = search_regime_pcm(state, cid, pcm_id, models, feature_cols, top_n=top_n,
                                     feasibility_feature_cols=feasibility_feature_cols)
            if not top.empty:
                all_top.append(top)
