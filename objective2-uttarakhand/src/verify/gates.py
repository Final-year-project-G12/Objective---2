...
from src.io_utils import load_system_config, load_design_bounds, get_pcm_properties, load_hourly_weather, load_demand_profile, get_regime
...
def gate3_baseline_comparison(state: str, log):
    ...
    cid = 0
    # Was hardcoded "n-Octacosane (C28)" with a comment claiming it's "the
    # Objective 1 consensus rank-1 PCM for every TN cluster" -- true for
    # Tamil Nadu, but silently wrong for any other state. Now reads the
    # ACTUAL rank-1 PCM for THIS state/cluster from states/<state>.yaml's
    # pcm_shortlist (index 0 = consensus_rank 1, per how build_input_package.py
    # / configs/states/<state>.yaml populate it from mcdm_topk_by_cluster.csv).
    ...
    # range (rather than this state's actual shortlisted PCM's own
    # climate-derived Tm) is plugged into the SAME geometry, does it beat plain tank? This isolates "is the
    ...
    tm_c = get_pcm_properties(state, pcm)["Tm_C"]
    log(f"\n  {pcm} (Objective 1's actual rank-1 PCM for this state/cluster, Tm={tm_c:.1f}C) beats plain tank "
    ...
    if not pcm_beats_plain:
        ...
        log(f"  tank size and PCM fraction, {pcm}'s mean liquid fraction stays very low")
        log(f"  (rarely reaches {tm_c:.1f}C), so it mostly displaces sensible-storage water without")
