...
era5-uttarakhand/ pipeline into this project's data/objective1/ folder, and
...
3. MEDOID RAW WEATHER — for each cluster, the single point with the
   highest GMM membership probability (cluster_assignments_uttarakhand.csv)
...
SMALL_FILES = [
    ...
    ("daily_aggregates_uttarakhand.csv", "daily_aggregates_uttarakhand.csv"),
    ("tier2_signature_uttarakhand.csv", "tier2_signature_uttarakhand.csv"),
    ("era5_power_agreement_uttarakhand.csv", "era5_power_agreement_uttarakhand.csv"),
    # Phase 3 — climate signature
    ("signatures/climate_signature_uttarakhand.csv", "climate_signature_uttarakhand.csv"),
    ("signatures/pca_loadings.csv", "pca_loadings.csv"),
    # Phase 4 — clustering / climate regimes
    ("clustering/cluster_assignments_uttarakhand.csv", "cluster_assignments_uttarakhand.csv"),
    ("clustering/cluster_profiles_uttarakhand.csv", "cluster_profiles_uttarakhand.csv"),
    ("clustering/bic_selection_uttarakhand.csv", "bic_selection_uttarakhand.csv"),
    ("clustering/kmeans_comparison_uttarakhand.csv", "kmeans_comparison_uttarakhand.csv"),
    # Phase 5-6 — PCM database, feasibility, MCDM ranking
    ("pcm/pcm_database_uttarakhand.csv", "pcm_database_uttarakhand.csv"),
...
LARGE_REFERENCE_FILES = [
    OBJ1_PROCESSED_DIR / "climate_uttarakhand_points.csv",
    OBJ1_PREPROCESSED_DIR / "uttarakhand_cleaned_physical.csv",
    OBJ1_PREPROCESSED_DIR / "uttarakhand_cleaned_scaled.csv",
]
...
def find_medoid_points():
    ...
    path = OBJ1_PROCESSED_DIR / "clustering" / "cluster_assignments_uttarakhand.csv"
    if not path.exists():
        print("  [WARN] cluster_assignments_uttarakhand.csv not found — run "
              "05_cluster_uttarakhand.py in the Objective 1 pipeline first. "
              "Skipping medoid raw-weather copy.")
        return []
    assign = pd.read_csv(path)
    if "max_membership_prob" not in assign.columns:
        print("  [WARN] cluster_assignments_uttarakhand.csv has no "
