...
PORTED FROM objective2-tamilnadu/src/optimize/search.py — search logic,
candidate count and ranking rule unchanged. Only the model path (this
repo's flat results/phase6_surrogate_models.pkl), the output path
(results/phase7_surrogate_top_candidates.csv) and the __main__ state
fallback differ.
"""
...
def search_all_pairs(state: str, top_n: int = 5):
    ...
    result.to_csv(TOP_CANDIDATES_PATH, index=False)
