...
def write_contract(state: str):
    ...
    contract_path = RESULTS_DIR / f"obj3_environment_contract_{state}.json"
    ...
    contract = {
        ...
        "control_skeleton": {
            ...
            "safety_shield": {
                ...
                "rationale": f"Phase 8 robustness found P(temp-safe) well below 0.95 for every "
                             f"{state.title()} regime even with the plain tank — an active bypass shield "
                             f"is a hard requirement here, not an optimisation nicety.",
            },
        },
        "dynamic_state_schema": _dynamic_state_schema(),
        "reset_scenarios": _reset_scenarios(sc),
        "weather_sequences": {
            "training_validation_test": f"NOT YET SPLIT — only one medoid weather year per regime exists "
                                        f"(data/weather/weather_regime_{state}_cluster<k>_hourly.csv); "
                                        f"Objective 3 must not assume separate train/val/test weather "
                                        f"years are already prepared (40-hr cut list).",
        },
