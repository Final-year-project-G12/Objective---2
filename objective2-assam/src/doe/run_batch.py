...
def get_sim_version(state: str) -> str:
    return f"sim_v1_{state}"
...
def run_case_spec(state: str, spec):
    ...
    t0 = time.time()
    out = run_case(state, spec.regime_id, spec.pcm_id, design, record_hourly=True)
    runtime_s = time.time() - t0

    row = {
        "case_id": spec.case_id, "state": state, "regime_id": spec.regime_id,
        ...
        "simulator_version": get_sim_version(state), "runtime_s": runtime_s,
...
def run_batch(state: str, n_lhs_per_pair: int = N_LHS_PER_PAIR_DEFAULT, progress_every: int = 20):
    ...
    print(f"Running {len(cases)} DOE cases for state={state} "
          f"(simulator={get_sim_version(state)}) ...")
    ...
    # Attach all 29 surrogate input features
    from src.surrogate.features import build_feature_table
    df = build_feature_table(state, df)
