...
Every figure is built once in Plotly and saved TWICE:
  results/plots/interactive/<name>.html   -- self-contained (all JS inlined),
                                             ...
                                             hover tooltips), opens in any
                                             browser with no server/internet.
  results/plots/static/<name>.png          -- same figure, flat image, for
                                             reports/slides/viva printouts.

Nothing here re-derives results — every figure either reads a file already
written by Phases 2-7 (phase5_design_cases.parquet, phase7_optimized_designs.csv,
the trained surrogate) or re-runs a specific, already-verified case through
run_case() to get a time series that isn't persisted by default (Phase 3).

PORTED FROM objective2-tamilnadu/src/plots/make_plots.py. The figure code
is unchanged; only the flat results/phaseN_* input paths, the 3-regime
...
def get_rank1_pcms(state: str) -> dict:
    ...
    for r in cfg.get("regimes", []):
        ...
        shortlist = r.get("pcm_shortlist", [])
        pcm_map[cid] = shortlist[0] if shortlist else None
    return pcm_map
...
def phase4_gate1_residuals(state, out_dir):
    ...
    cases = [
        (f"A: cluster0/{pcm_c0}", 0, pcm_c0, DesignVector(0.05, 14, 0.030)),
        (f"B: cluster1/{pcm_c1}", 1, pcm_c1, DesignVector(0.04, 20, 0.020)),
        (f"C: cluster2/{pcm_c2}", 2, pcm_c2, DesignVector(0.08, 10, 0.045)),
        ("D: cluster2/no-PCM", 2, None, DesignVector(0.05, 14, 0.030)),
        (f"E: cluster0/bounds-extreme", 0, pcm_c0, DesignVector(0.08, 24, 0.050)),
    ]
    names, residuals = [], []
...
def phase5_doe_coverage(state, out_dir, design_cases):
    ...
    fig.update_layout(title=f"Phase 5 — DOE sample coverage (165 cases: LHS + boundary + baseline) — {state}",
...
def phase5_outcome_distribution(state, out_dir, design_cases):
    ...
    fig.add_trace(go.Histogram(x=valid["solar_fraction"], marker_color="#ff7f0e", showlegend=False),
                  row=1, col=2)
    fig.update_layout(title=f"Phase 5 — outcome distribution across 111 valid DOE cases — {state}")
...
def phase6_parity_plots(state, out_dir, design_cases):
    ...
    with open(SURROGATE_MODELS_PATH, "rb") as f:
        saved = pickle.load(f)
...
def phase7_pareto_by_regime(state, out_dir, optimized, deployable):
    regimes = sorted(optimized["regime_id"].unique())
    fig = make_subplots(rows=1, cols=len(regimes), subplot_titles=[f"Regime {r}" for r in regimes])
    positions = [(1, c) for c in range(1, len(regimes) + 1)]
    ...
    _save(fig, "phase7_pareto_by_regime", out_dir, width=1200, height=480)
...
def phase8_robustness_probabilities(state, out_dir, robustness_summary):
    ...
    n_draws = int(robustness_summary["n_draws"].iloc[0]) if "n_draws" in robustness_summary.columns else 100
    fig = go.Figure()
    ...
    fig.update_layout(barmode="group",
                       title=f"Phase 8 — robustness probabilities ({n_draws} Monte Carlo draws/design) — {state}",
...
def phase8_useful_energy_intervals(state, out_dir, robustness_summary, deployable):
    ...
    for _, row in robustness_summary.iterrows():
        ...
        fig.add_trace(go.Scatter(
            ...
            name="P50 (median)", showlegend=bool(cid == int(robustness_summary["regime_id"].iloc[0])),
        ...
        if not nominal_row.empty:
            fig.add_trace(go.Scatter(
                ...
                name="nominal (Phase 7)", showlegend=bool(cid == int(robustness_summary["regime_id"].iloc[0])),
    ...
    fig.update_layout(title=f"Phase 8 — useful-energy 5th-50th-95th percentile interval per regime "
                             f"({n_draws} Monte Carlo draws) — {state}",
