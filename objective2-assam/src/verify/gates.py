...
def get_rank1_pcms(state: str) -> dict:
    """Dynamically resolve rank-1 PCM for each regime from states/<state>.yaml."""
...
def gate1_conservation(state: str, log):
    ...
    cases = [
        (f"A: cluster0 / {pcm_c0} / mid design", 0, pcm_c0, DesignVector(0.05, 14, 0.030)),
        (f"B: cluster1 / {pcm_c1} / small-capsule design", 1, pcm_c1, DesignVector(0.04, 20, 0.020)),
        (f"C: cluster2 / {pcm_c2} / large-capsule design", 2, pcm_c2, DesignVector(0.08, 10, 0.045)),
        ("D: cluster2 / no-PCM plain-tank baseline", 2, None, DesignVector(0.05, 14, 0.030)),
        (f"E: cluster0 / {pcm_c0} / bounds-extreme design", 0, pcm_c0, DesignVector(0.08, 24, 0.050)),
    ]
...
def gate2_limiting_cases(state: str, log):
    ...
    # 13. INFORMATIONAL (per-state finding):
    ...
    #     the verification report, whether Cluster 0 overheats the tank on solar
    ...
    checks.append((f"{state}_cluster0_plain_tank_overheat (informational, non-gating)", True,
...
def gate3_baseline_comparison(state: str, log):
    ...
    cid = 0
    pcm_map = get_rank1_pcms(state)
    pcm = pcm_map.get(0, "RT50")   # Objective 1 rank-1 PCM for Cluster 0
...
def run_all_gates(state: str):
    ...
    report_path = RESULTS_DIR / f"phase4_simulator_verification_report_{state}.txt" if state != "rajasthan" else RESULTS_DIR / "phase4_simulator_verification_report.txt"
