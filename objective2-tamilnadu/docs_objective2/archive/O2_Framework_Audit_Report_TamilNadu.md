> **ARCHIVED 2026-09-17.** This audit predates two further changes: (1)
> the Objective 1 data refresh (GMM regime count K=5→K=3, real elevation,
> rewritten MCDM/feasibility engine — see
> `docs_objective2/16_OBJECTIVE1_DATA_REFRESH.md`) and (2) capsule
> arrangement being restored as a searched design variable (see
> `docs_objective2/17_ARRANGEMENT_RESTORATION.md`). Every regime, PCM
> shortlist, and numeric result this audit discusses is from the
> pre-refresh K=5/staggered-only run and no longer corresponds to the
> current codebase. Kept for historical record of the methodology issues
> it identified (most now resolved, some already noted below as resolved
> by the 2026-09-14 revisions) — not a current-state reference. For
> current results, see `docs_objective2/00_MASTER_OVERVIEW.md` and the
> Phase docs in `docs_objective2/tamilnadu_phase_docs/`.
...
> **⚠️ Status update (2026-09-14): five methodology revisions applied
...
> (`docs_objective2/13_DESIGN_BOUNDS_WIDENING.md`), (3) the final
...
> (`docs_objective2/14_SELECTION_RULE_SCOPE_CORRECTION.md`), and (4/5) the
> PCM shortlist was re-derived using Objective 1's real 4-method MCDM
> engine (replacing a simplified nearest-Tm substitute) and the selection
> tie-break was corrected to prefer temperature-safe candidates
> (`docs_objective2/15_MCDM_RERANKING_AND_SAFETY_TIEBREAK.md`). **Result:
> every regime's Objective 2 recommendation is now a genuine PCM design,
> beating plain tank on useful energy in all 5 regimes** (+0.04% to
> +0.30%), and the temperature-safety finding (Part 3.2/4 below) is more
> nuanced than either earlier snapshot: P(temp-safe) is 0% in regimes 0–3
> and **30.8%** in regime 4 (down from the pre-correction 71.7–92.5%
> plain-tank-dominated figures below, but a real improvement over an
> intermediate, now-superseded regime-4 pick whose margin was razor-thin),
> because a PCM design is mechanically exposed to a tighter (65°C)
> temperature ceiling than a plain tank's single 75°C limit. Part 3's
> discovery-chain narrative and the specific numbers in
...
**Current (2026-09-14, all 5 regimes now PCM, real-MCDM shortlist +
safety-first tie-break — `docs_objective2/10_PHASE8_ROBUSTNESS_HANDOFF.md`,
`docs_objective2/15_MCDM_RERANKING_AND_SAFETY_TIEBREAK.md`):**

| Regime | PCM | P(meets demand) | P(temp-safe) | Max water T P95 |
|---|---|---|---|---|
| 0 | n-Tetracosane (C24) | 80.8% | **0%** | 75.0°C |
| 1 | n-Tetracosane (C24) | 90.8% | **0%** | 78.4°C |
| 2 | n-Hexacosane (C26) | 94.2% | **0%** | 78.1°C |
| 3 | n-Hexacosane (C26) | 98.3% | **0%** | 78.0°C |
| 4 | **RT45HC** | 76.7% | **30.8%** | 74.3°C |

The safety picture for regimes 0–3 is uniformly worse than the original,
...
> **Update, 2026-09-14:** the Tamil Nadu column below is the
> pre-scope-correction snapshot (mixed plain-tank/PCM selection). Current,
> all-PCM Tamil Nadu numbers (after the real-MCDM shortlist + safety-first
> tie-break, doc 15): P(temp-safe) ranges **0% (regimes 0–3) to 30.8%
> (regime 4)** — every regime still fails the 95% bar, and by a wider
> margin than the table below shows for regimes 0–3, since every design is
> now PCM and therefore subject to the tighter dual-limit constraint;
> regime 4 is the exception, now with a real (not razor-thin) safety
> margin. The comparison's qualitative conclusion (below) is unchanged and,
> if anything, stronger: both states need active overheat protection, and
> Tamil Nadu's own post-correction numbers now make that case using its
> actual PCM recommendation rather than a partially plain-tank one.
...
- "3.3 Optimized designs per climate regime" (5 designs, energy/mass/pump
  trade-off table — **updated 2026-09-14: all 5 now select a genuine PCM
  design**, beating plain tank on useful energy by +0.04–0.30% in every
  regime (regime 4 also now genuinely temperature-safe, +0.39°C margin),
  per `docs_objective2/14_SELECTION_RULE_SCOPE_CORRECTION.md` and
  `docs_objective2/15_MCDM_RERANKING_AND_SAFETY_TIEBREAK.md`)
...
**As of 2026-09-14, Component 1 is resolved** (Tm-target retargeting +
bounds widening + a selection-rule scope correction + a real-MCDM
shortlist and safety-first tie-break — docs 12–15 — together make PCM the
genuine, simulator-confirmed winner in all 5 regimes), and **Component 2
is not only unresolved but sharper, though regime 4 is now genuinely
better-protected**: every regime's actual PCM recommendation is 0–30.8%
temperature-safe under uncertainty (regime 4 with a real, not razor-thin,
nominal margin), a stronger and more concrete version of the original
caveat because it now describes the system Objective 2 actually
recommends, not a partially plain-tank stand-in. Read together with Rajasthan's
