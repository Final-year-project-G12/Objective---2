...
**Status: APPLIED to `configs/states/tamilnadu.yaml`. Phases 5–8 fully re-run against it, together with the design-bounds widening (doc 13), the selection-rule scope correction (doc 14), and the full-MCDM shortlist adoption + safety tie-break (doc 15) — see `RESULTS.md` for final, current numbers. The "What has and hasn't been re-run" and "Headline finding" sections below are kept as a historical record of the intermediate (retargeted-only, pre-widening, pre-scope-correction) state; they are superseded by docs 13–15. IMPORTANT: the "Result: before → after" shortlist table below (§below) is ALSO superseded — those shortlists used a simplified nearest-Tm-distance ranking that doc 15 found disagreed with Objective 1's actual MCDM method in every regime; the `Tm_target_C` values themselves are still current.**
